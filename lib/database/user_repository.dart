...
class UserRepository {
  ...
  Future<Map<String, dynamic>> signIn(Map<String, dynamic> user) async {
    ...
    try {
      ...
      var decoded = json.decode(response.body);
      if (response.statusCode >= 200 && response.statusCode < 300) {
        Map<String, dynamic> responseUser = await UserRepository().getMyUserDetails(decoded["token"]);
  ...
  Future<Map<String, dynamic>> getAllUsersDetails(String me) async {
    ...
    try {
      http.Response response = await http.get(uri, headers: headers);
      if (response.statusCode >= 200 && response.statusCode < 300) {
        List<UserDetails> allWithoutMe = listUsersFromJson(response.body).where((element) => element.userId != me).toList();
